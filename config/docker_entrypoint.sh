...
sed -i "s/docker.example.com/$HOSTNAME/g" /etc/httpd/conf/httpd.conf
sed -i "s/docker.example.com/$HOSTNAME/g" /etc/httpd/conf.d/ssl.conf
...
if [[ -z "$AGAVE_API_PORT" ]]; then
  export AGAVE_BASE_URL="https://$HOSTNAME/v2"
else
  export AGAVE_BASE_URL="https://$HOSTNAME:$PORT/v2"
fi
